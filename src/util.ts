...
// 1) 文件相关
export const DirToJson = (dirPath: string, obj: Record<string, Record<string, string> | string> = {}, exclude?: string[]) => {
  for (const file of fs.readdirSync(dirPath, { withFileTypes: true })) {
    // 1 移除无用文件
    if (exclude?.some(v => file.name.includes(v))) continue;
    // 2 设置文件夹
    if (file.isDirectory()) {
      obj[file.name] = {}
      DirToJson(`${dirPath}/${file.name}`, obj[file.name] as Record<string, string>, exclude)
    }
    // 3 设置文件
    else {
...
export const judgeType = (filePath: string): "file" | "folder" => {
  return extname(filePath) ? "file" : "folder"
}
